...
- Qt 6.11.1：`D:/Qt/6.11.1/mingw_64/`
- MinGW 13.1.0：`D:/Qt/Tools/mingw1310_64/`
- Qt Creator：`D:/Qt/Tools/QtCreator/bin/qtcreator.exe`

注意：系统自带 MinGW 8.1.0（`/d/Mingw/mingw64/`）与 Qt 不兼容，**必须使用 Qt 自带的 MinGW 13.1.0**。
...
PATH="/d/Qt/Tools/mingw1310_64/bin:/d/Qt/6.11.1/mingw_64/bin:/usr/bin:/bin"

# 编译（每次 git pull 之后执行）
"D:/Qt/6.11.1/mingw_64/bin/qmake.exe" "what_to_eat.pro" -o Makefile
...
cp dishes.json synonyms.json zones.json distances.json background.png \
   ...
   map_origin.jpg start_page_background.png title-art.png \
   "久石让+-+Summer+-+菊次郎的夏天+-+钢琴独奏版.mp3" \
   "11724.mp3" release/
"D:/Qt/6.11.1/mingw_64/bin/windeployqt.exe" release/what_to_eat.exe
```
...
3. 配置 Kit：选择 **Desktop Qt 6.11.1 MinGW 64-bit**
4. 左下角切换到 **Release** 模式
...
- C++17，Qt 6.11 Widgets（手写布局，无 .ui 文件）
